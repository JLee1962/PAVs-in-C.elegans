...
faToTwoBit /mnt/3main/lotusbo/isoseq.analysis/resources/pd1074/c_elegans.PRJEB28388.WS274.genomic.rename.fa \
    /mnt/3main/lotusbo/isoseq.analysis/resources/pd1074/pd1074.2bit
faToTwoBit /mnt/3main/lotusbo/isoseq.analysis/resources/cb4856/cb4856.scaffold.gapfilling.mt.wrapped.rename.fasta \
    /mnt/3main/lotusbo/isoseq.analysis/resources/cb4856/cb4856.2bit

mkdir /mnt/3main/lotusbo/isoseq.analysis/liftover/
mkdir /mnt/3main/lotusbo/isoseq.analysis/liftover/pdtocb


# I modified SameSpeciesBlatSetup script from http://genomewiki.ucsc.edu/images/9/91/SameSpeciesBlatSetup.sh.txt
#
...
export targetChunkSize="22000000"
...
export queryChunkSize="22000000"
...
rm -fr tParts
twoBitInfo ${twoBitDirectory}/${targetDb}/${targetDb}.2bit stdout | sort -k2nr > ${targetDb}.chrom.sizes
...
$HOME/kent/src/hg/utils/automation/partitionSequence.pl ${targetChunkSize} \
   ${chunkOverlapSize} ${twoBitDirectory}/${targetDb}/${targetDb}.2bit ${targetDb}.chrom.sizes \
   ${chunkCountLimit} \
   -lstDir=tParts > t.lst
...
rm -fr qParts
twoBitInfo ${twoBitDirectory}/${queryDb}/${queryDb}.2bit stdout | sort -k2nr > ${queryDb}.chrom.sizes
# check the usage message for this command in the kent source tree
#   to see what all the arguments mean:
$HOME/kent/src/hg/utils/automation/partitionSequence.pl ${queryChunkSize} \
   ${chunkOverlapSize} ${twoBitDirectory}/${queryDb}/${queryDb}.2bit ${queryDb}.chrom.sizes \
   ${chunkCountLimit} \
   -lstDir=qParts > q.lst
...
export realSize=`twoBitToFa ${twoBitDirectory}/${targetDb}/${targetDb}.2bit stdout | faSize stdin | grep real | awk '{print $5}'`
...
blat ${twoBitDirectory}/${targetDb}/${targetDb}.2bit /dev/null \
