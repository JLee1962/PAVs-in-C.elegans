...
mkdir /mnt/3main/lotusbo/isoseq.analysis/cb4856/sqanti3/novel_gene
export comD="/mnt/3main/lotusbo/isoseq.analysis/cb4856"
export workD="/mnt/3main/lotusbo/isoseq.analysis/cb4856/sqanti3/novel_gene"

cat ${comD}/sqanti3/sqanti3_qc/cb4856.collapsed.filtered.rep_classification.txt | awk '{ if (index($7,"novelGene")!=0) print $0}' \
    >> ${workD}/novel_gene_candidate_list

list=`cat ${workD}/novel_gene_candidate_list | awk '{print $1}'`
for i in $list; do
    cat ${comD}/isoseq3.one/collapse/cb4856.collapsed.read_stat.txt | grep "$i$" >> ${workD}/novel_gene_read_list
done

list=`cat ${workD}/novel_gene_candidate_list | awk '{print $1}' | sed 's/$/";/g'`
for i in $list; do
    cat ${comD}/sqanti3/sqanti3_qc/cb4856.collapsed.filtered.rep_corrected.gtf.cds.gff | grep $i >> ${workD}/novel_gene_candidate.gff
done

samtools view ${comD}/isoseq3.one/ccs/cb4856.isoseq.ccs.bam > ${comD}/isoseq3.one/ccs/cb4856.isoseq.ccs.sam

list=`cat ${workD}/novel_gene_read_list | awk '{print $1}'`
for i in $list; do
    echo $i | awk -v i="$i" '{ if ($1==i) {print ">"$1; print $10}}' ${comD}/isoseq3.one/ccs/cb4856.isoseq.ccs.sam >> ${workD}/novel_gene_ccs.fa
done

minimap2 -ax splice -t 20 /mnt/3main/lotusbo/isoseq.analysis/resources/cb4856/cb4856.scaffold.gapfilling.mt.wrapped.rename.fasta ${workD}/novel_gene_ccs.fa \
    | samtools sort -o ${workD}/novel_gene.bam
samtools index ${workD}/novel_gene.bam

# 02_Comparing newly found gene candidates with N2, known pd1074, long-read pd1074 transcripts

mkdir ${workD}/blast
liftOver -gff /mnt/3main/lotusbo/isoseq.analysis/pd1074/isoseq3.one/collapse/pd1074.collapsed.filtered.gff \
    /mnt/3main/lotusbo/isoseq.analysis/liftover/pdtocb/pd1074Tocb4856.over.chain.gz \
    /mnt/3main/lotusbo/isoseq.analysis/cb4856/isoseq3.one/liftOver/pd1074.collapsed.filtered.ptcliftOver.gff \
    /mnt/3main/lotusbo/isoseq.analysis/cb4856/isoseq3.one/liftOver/pd1074.collapsed.filtered.ptcunmapped.gff

sed 's/PB/PD.PB/g' /mnt/3main/lotusbo/isoseq.analysis/cb4856/isoseq3.one/liftOver/pd1074.collapsed.filtered.ptcliftOver.gff \
    >> /mnt/3main/lotusbo/isoseq.analysis/cb4856/isoseq3.one/liftOver/pd1074.collapsed.filtered.ptcliftOver.rename.gff
...
blastn -query ${workD}/novel_gene_ccs.fa -db /mnt/3main/lotusbo/isoseq.analysis/resources/blast_db/pd1074_isoseq3/pd1074_isoseq3 \
    -outfmt  "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen qcovs" \
    -num_threads 20 -out ${workD}/blast/novel_gene_ccs.blast.pd1074_isoseq3.out
...
list=`awk '{print $1}' ${workD}/blast/novel_gene_ccs.blast.pd1074_isoseq3.out | sort -u`
for i in $list ; do
    awk -v i="$i" '$1==i {print $5}' ${workD}/novel_gene_read_list >> ${workD}/blasted.txt
done
...
bioawk -c fastx '{ print $name"\t"length($seq) }' ${comD}/sqanti3/sqanti3_qc/cb4856.collapsed.filtered.rep_corrected.fasta \
    > ${comD}/sqanti3/sqanti3_qc/cb4856.collapsed.filtered.rep_corrected.fasta.length

echo -e "name\tlength\tnumberofexon\tchr\tstart\tend\tabun_num\tabun_rate" >> ${workD}/novel.spec/notblasted.novel_gene_candidate_len_exon.txt
list=`awk '{print $1}' ${workD}/notblasted.novel_gene_candidate_list.txt`
for i in $list; do
    namelen=`awk -v i="$i" '$1==i' ${comD}/sqanti3/sqanti3_qc/cb4856.collapsed.filtered.rep_corrected.fasta.length`
    j=$i\"\;
    ...
    abun=`awk -v i="$i" '$1==i {print $2"\t"$3}' ${comD}/isoseq3.one/collapse/cb4856.collapsed.abundance.txt`
    echo -e "$namelen\t$numofexon\t$chrstartend\t$abun" >> ${workD}/novel.spec/notblasted.novel_gene_candidate_len_exon.txt
done


# 03. Extracting the protein sequence of newly found genes for blastp search and Batch CD-search
gffread -y ${workD}/novel.spec/notblasted.novel_gene_candidate.protein.faa -g /mnt/3main/lotusbo/isoseq.analysis/resources/cb4856/cb4856.scaffold.gapfilling.mt.wrapped.rename.fasta \
